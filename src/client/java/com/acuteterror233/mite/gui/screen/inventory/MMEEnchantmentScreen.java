...
public class MMEEnchantmentScreen extends AbstractContainerScreen<MMEEnchantmentMenu> {
        ...
        for (int n = 0; n < 3; n++) {
            ...
            if (q == 0) {
                ...
                if ((m < n + 1 || this.minecraft.player.experienceLevel < q) && !this.minecraft.player.hasInfiniteMaterials()) {
                    ...
                    guiGraphics.drawWordWrap(this.font, formattedText, p, l + 16 + 19 * n, r, (s & 16711422) >> 1, false);
                    s = 4226832;
                } else {
                    int t = i - (k + 60);
                    int u = j - (l + 14 + 19 * n);
    ...
    private void renderBook(GuiGraphics guiGraphics, int i, int j, float f) {
        ...
        guiGraphics.pose().translate(i + 33.0F, j + 31.0F, 100.0F);
        float k = 40.0F;
        ...
        for (int l = 0; l < 3; l++) {
            ...
            if (!optional.isEmpty()) {
                ...
                if (this.isHovering(60, 14 + 19 * l, 108, 17, i, j) && m > 0 && n >= 0 && optional != null) {
                    ...
                    list.add(
                            Component.translatable("container.enchant.clue", new Object[]{Enchantment.getFullname((Holder)optional.get(), n)}).withStyle(ChatFormatting.WHITE)
                    );
                    if (!bl) {
