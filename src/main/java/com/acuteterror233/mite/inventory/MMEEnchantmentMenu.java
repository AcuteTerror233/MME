...
public class MMEEnchantmentMenu extends AbstractContainerMenu {
    ...
    private final Container enchantSlots = new SimpleContainer(2) {
        ...
        public void setChanged() {
            super.setChanged();
            MMEEnchantmentMenu.this.slotsChanged(this);
        }
    };
        ...
        if (container == this.enchantSlots) {
            ...
            if (!itemStack.isEmpty() && itemStack.isEnchantable()) {
                this.access.execute((level, blockPos) -> {
                    ...
                    for (int j = 0; j < 3; j++) {
                        this.costs[j] = EnchantmentHelper.getEnchantmentCost(this.random, j, enchantmentLevel, itemStack);
        ...
        if (i >= 0 && i < this.costs.length) {
            ...
            if ((itemStack2.isEmpty() || itemStack2.getCount() < j) && !player.hasInfiniteMaterials()) {
                return false;
                    ...
                    || itemStack.isEmpty()
                    || (player.experienceLevel < j || player.experienceLevel < this.costs[i]) && !player.hasInfiniteMaterials()) {
                return false;
            } else {
                this.access.execute((level, blockPos) -> {
